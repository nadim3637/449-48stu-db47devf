...
import { AiTutorTrigger } from './AiTutorTrigger';
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating, onNavigateToChapter }) => {
              ...
              <div className="space-y-6 pb-24">
                ...
                {/* HERO SECTION */}
                  <div className="space-y-4 mb-6">
                      ...
                      {/* SPECIAL DISCOUNT BANNER (Kept Separate) */}
                      ...
                      {/* STACKED BANNERS CAROUSEL */}
                      <BannerCarousel banners={[
                          // 1. AI Notes Poster (AI Banner)
                          settings?.isAiEnabled && (
                              <div 
                                  key="ai_poster"
                                  onClick={() => setShowAiModal(true)}
                                  className="w-full h-full relative overflow-hidden rounded-none shadow-none cursor-pointer group bg-gradient-to-r from-violet-600 to-indigo-600 p-5 flex items-center justify-between"
                              >
                          ...
                          // 2. Hero Slider
                          <DashboardSectionWrapper key="hero_slider" id="hero_slider" label="Hero Slider">
                              <div className="relative h-48 w-full overflow-hidden" style={{ backgroundColor: 'var(--primary)' }}>
                          ...
                          // 3. Custom Page Banner
                          <div 
                              key="custom_page"
                              onClick={() => onTabChange('CUSTOM_PAGE')}
                              className="w-full h-full bg-gradient-to-r from-orange-500 to-red-500 p-4 flex items-center justify-between cursor-pointer relative overflow-hidden group"
                          >
                              <div className="relative z-10">
                          ...
                          // 4. Morning Insight
                          morningBanner && (
                              <div key="morning_insight" className="w-full h-full bg-gradient-to-r from-orange-100 to-amber-100 p-4">
                          ...
                          // 5. Live Challenges
                          <DashboardSectionWrapper key="live_challenges" id="live_challenges" label="Live Challenges">
                              <div className="w-full h-full bg-slate-900 p-4 relative overflow-hidden">
                                  ...
                                  <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide relative z-10">
                                      ...
                                      {challenges20.map(c => {
                                          ...
                                          const now = Date.now();
                                          const timeLeft = Math.max(0, Math.floor((expiry.getTime() - now) / (1000 * 60)));
                          ...
                          // 6. Features Ticker
                          <DashboardSectionWrapper key="features_ticker" id="features_ticker" label="Features Ticker">
                          ...
                          // 7. Promo Banner
                          <DashboardSectionWrapper key="promo_banner" id="promo_banner" label="Promo Banner">
                              <div onClick={() => onTabChange('STORE')} className="w-full h-full bg-gradient-to-r from-slate-900 to-slate-800 p-4 flex items-center justify-between cursor-pointer relative overflow-hidden group">
                                  <div className="relative z-10">
                                      <div className="flex items-center gap-2 mb-1">
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-lg z-50 pb-safe">
            <div className="flex justify-around items-center h-16">
                ...
                <div className="w-full h-full p-1">
                    <AiTutorTrigger onClick={() => setShowAiTutor(true)} />
                </div>
            </div>
        </div>
        ...
        <StudentAiAssistant user={user} settings={settings} isOpen={showAiTutor} onClose={() => setShowAiTutor(false)} />
    </div>
  );
};
